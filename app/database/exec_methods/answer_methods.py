...
async def fill_unanswered_feedback(
    fb_id: str,
    fb_rating: int,
    fb_shop_wb: str,
    fb_product_wb: str,
    ...
    await cursor.execute(
        "INSERT INTO feedbacks (fb_id, fb_rating, fb_shop_wb, fb_product_wb, fb_text, fk_api_key) VALUES (?, ?, ?, ?, ?, ?)",
...
async def get_feedback(fb_id: str, db=path) -> bool | list:
    ...
    await cursor.execute(
        "SELECT fb_rating, fb_shop_wb, fb_product_wb, fb_text FROM feedbacks WHERE fb_id=?",
        (fb_id,),
    )
...
async def get_unanswered_fb_list(telegram_id: int, db=path) -> bool | list:
    ...
    await cursor.execute(
        "SELECT fb.fb_rating, fb.fb_shop_wb, fb.fb_product_wb, fb.fb_text, fb.fb_id, fb.fk_api_key FROM feedbacks fb LEFT JOIN shops sh on sh.api_key=fb.fk_api_key LEFT JOIN users u on u.tg_id=sh.fk_tg_id WHERE u.tg_id=? AND fb.fb_answer IS NULL",
        (telegram_id,),
    )
...
async def get_answer(telegram_id: int, shop_name: str, db=path) -> str | bool:
    ...
    await cursor.execute(
        "SELECT fb.fb_answer FROM feedbacks fb LEFT JOIN shops sh on sh.api_key=fb.fk_api_key LEFT JOIN users u on u.tg_id=sh.fk_tg_id WHERE u.tg_id=? AND sh.shop_name=?",
...
async def get_not_null_answer_feedbacks_list(telegram_id: int, db=path) -> list | bool:
    ...
    await cursor.execute(
        "SELECT fb.fb_rating, fb.fb_shop_wb, fb.fb_product_wb, fb.fb_text, fb.fb_answer, fb.fb_id FROM feedbacks fb LEFT JOIN shops sh on sh.api_key=fb.fk_api_key LEFT JOIN users u on u.tg_id=sh.fk_tg_id WHERE u.tg_id=? AND fb.fb_answer IS NOT NULL",
        (telegram_id,),
    )
...
async def update_answer_text(fb_id: str, ans_text: str, db=path):
    ...
    if ans_text == "null":
        await cursor.execute(
            "UPDATE feedbacks SET fb_answer=NULL WHERE fb_id=?", (fb_id,)
        )
...
async def mark_shown_feedback(fb_id: str, db=path):
    ...
    await cursor.execute(
        "UPDATE feedbacks SET show_date=datetime('now', '+3 hours') WHERE fb_id=? AND show_date=0",
        (fb_id,),
    )
...
async def delete_old_shown_feedback(db=path):
    ...
    await cursor.execute(
        "DELETE FROM feedbacks WHERE datetime('now', '+3 hours') > datetime(show_date, '+24 hours') AND show_date != 0"
    )
...
async def delete_feedback(fb_id: str, db=path):
    conn = await create_connection(db)
    cursor = await conn.cursor()

    await cursor.execute("DELETE FROM feedbacks WHERE fb_id=?", (fb_id,))
