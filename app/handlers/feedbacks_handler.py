...
router_answers.include_router(router_unanswered)
...
async def show_archive(callback_query: CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.edit_text(
        text=f"Я ответил на {got_counter} отзывов! Все в порядке, я справляюсь, Босс)",
...
async def show_last_5(callback_query: CallbackQuery, state: FSMContext):
    ...
    got_list_of_answered_fbs = await get_not_null_answer_feedbacks_list(
        callback_query.from_user.id
    )

    if not got_list_of_answered_fbs:
        await callback_query.message.edit_text(
            text="увы нет отвеченных отзывов", reply_markup=go_to_main_menu_keyboard
        )
        return
